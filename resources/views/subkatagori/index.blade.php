...
<section class="content">
    ...
    @endif
              ...
              <table id="datatable" class="table table-bordered table-hover">
                ...
                <tbody>
                  ...
                  <tr>
                    <td>{{$d->id}}</td>
                    <td>{{$d->nama_subkatagori}}</td>
                    <td>{{$d->katagori->nama_katagori}}</td>
                    <td>{{$d->keterangan}}</td>
                    <td>{{$d->aktif}}</td>
                    <td><center><a href="{{url('/sub_katagori/'.$d->id.'/edit')}}" class="btn btn-warning btn-sm">Edit</a></center></td>
                  </tr>
                  @endforeach
                </tbody>
                <tfoot>
  ...
  <div class="modal-dialog modal-lg">
    ...
    <form action="{{('/sub_katagori/do_create')}}" method="POST">
            {{csrf_field()}}
       
         <div class="form-group">
          <label for="exampleInputEmail1">Nama Sub Katagori</label>
          <input type="text" class="form-control" name="nama_subkatagori" id="nama" aria-describedby="emailHelp" placeholder="Nama Sub Katagori">
        </div>
        <div class="form-group">
